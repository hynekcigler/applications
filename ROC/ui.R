...
fluidRow(  column(
  ...
  sliderInput("prevalence", label = "Kolik procent lidí je skutečně nakažených?", min = 0, max = 100, value = 5, step = .1, 
              ...
              post=" %"),
  sliderInput("populace", label = "Počet osob v populaci", ticks = F,
              min = 100000, max = 11000000, value = 10649800, step = 100000),
  ...
  conditionalPanel(
    ...
    sliderInput("senz", label = "Senzitivita testu:", min = .5, max = 1, value = .7, step = .01, 
                animate=animationOptions(interval = 200, loop = T)), 
    sliderInput("spec", label = "Specificita testu:", min = .5, max = 1, value = .8, step = .01, 
                animate=animationOptions(interval = 200, loop = T))),
